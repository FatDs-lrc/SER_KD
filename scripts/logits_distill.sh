...
for i in `seq 1 1 10`;
do
cmd="python train_baseline.py --dataset_mode=iemocap_10fold --model=logits_distill
...
--temperature=2 --ce_weight=1.0 --kd_weight=1.0
...
--name=logits_distill --suffix=T{T_modality}_S{S_modality}_run{run_idx}
--cvNo=$i"
