...
def make_all_comparE(config):
    ...
    save_dir = os.path.join(config['feature_root'], 'feature', 'comparE_raw')
...
def migrate_comparE_to_npy(config):
    max_len = 600
    feat_path = os.path.join(config['feature_root'], 'feature', 'comparE_raw', 'all.h5')
    mean_std_path = os.path.join(config['feature_root'], 'feature', 'comparE_raw', 'mean_std.h5')
    feat_h5f = h5py.File(feat_path, 'r')
    mean_std = h5py.File(mean_std_path, 'r')
    for cv in range(1, 11):
        save_dir = os.path.join(config['feature_root'], 'feature', 'comparE_raw', str(cv)) # f'/data3/lrc/Iemocap_feature/cv_level/feature/comparE/{cv}'
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
...
def statis_comparE(config):
    path = os.path.join(config['feature_root'], 'feature', 'comparE_raw', 'all.h5')
...
if __name__ == '__main__':
    ...
    # normlize_on_trn(config, os.path.join(config['feature_root'], 'feature', 'comparE_raw', 'all.h5'), os.path.join(config['feature_root'], 'feature', 'comparE_raw', 'mean_std.h5'))
