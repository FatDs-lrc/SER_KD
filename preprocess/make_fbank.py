...
class FbankExtractor(object):  
    def __init__(self, num_bins=80, sample_frequency=16000, window=0.025, shift=0.01):
...
def make_all_fbank(config):
    ...
    save_dir = os.path.join(config['feature_root'], 'feature', 'fbank')
...
def migrate_fbank_to_npy(config):
    max_len = 600
    feat_path = os.path.join(config['feature_root'], 'feature', 'fbank', 'all.h5')
    mean_std_path = os.path.join(config['feature_root'], 'feature', 'fbank', 'mean_std.h5')
    feat_h5f = h5py.File(feat_path, 'r')
    mean_std = h5py.File(mean_std_path, 'r')
    for cv in range(1, 11):
        save_dir = os.path.join(config['feature_root'], 'feature', 'fbank', str(cv)) # f'/data3/lrc/Iemocap_feature/cv_level/feature/fbank/{cv}'
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
...
def statis_fbank(config):
    path = os.path.join(config['feature_root'], 'feature', 'fbank', 'all.h5')
...
if __name__ == '__main__':
    ...
    make_all_fbank(config)
    statis_fbank(config)
    normlize_on_trn(config, os.path.join(config['feature_root'], 'feature', 'fbank', 'all.h5'), os.path.join(config['feature_root'], 'feature', 'fbank', 'mean_std.h5'))
    migrate_fbank_to_npy(config)