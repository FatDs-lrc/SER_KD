...
class SincNetCNN(nn.Module):

    def __init__(self, options=None):
        super(SincNetCNN, self).__init__()
        ...
        for i in range(self.N_cnn_lay):
            ...
            self.ln.append(LayerNorm(
                [N_filt, int((current_input-self.cnn_len_filt[i]+1)/self.cnn_max_pool_len[i])]))

            self.bn.append(nn.BatchNorm1d(N_filt, int(
                (current_input-self.cnn_len_filt[i]+1)/self.cnn_max_pool_len[i]), momentum=0.05))
...
class SincNetEncoder(nn.Module):
    def __init__(self):
        ...
        cnn_options = self.load_default_cfg_cnn()
        self.cnn = SincNetCNN(cnn_options)
        dnn_options = self.load_default_cfg_dnn()
